 ...
 */
var fs = require('fs');
var arraySource = require('./PC data200-3000- vc 2018+par14+pres+rural.json')
var delegationData = require('./cleanedUnemployedPercentage delegation.json')
var log_file = fs.createWriteStream(__dirname + '/PC data200-3000- vc 2018+par14+pres+rural+unemployment.json', { flags: 'w' });
...
for (let i = 0; i < arraySource.length; i++) {
    ...
    for (let j = 0; j < delegationData.length; j++) {
        ...
        if (inside([lon, lat], polygon)) {
            ...
            var unemploymentPersentage = element2.properties.unemploymentPersentage
            element.unemploymentPercentage=unemploymentPersentage
            break;
        }else{
            element.unemploymentPercentage=null
