 ...
 */
var fs = require('fs');
var arraySource = require('./Sooo Final data200-3000- vc 2018+par14+pres+.json')
var delegationData = require('./cleaned rural urban per deleg ins data.json')
var log_file = fs.createWriteStream(__dirname + '/PC+ruralData.json', { flags: 'w' });
...
for (let i = 0; i < arraySource.length; i++) {
    ...
    for (let j = 0; j < delegationData.length; j++) {
        ...
        if (inside([lon, lat], polygon)) {
            ...
            var ruralPer = element2.properties.ruralPer
            var urbanPer = element2.properties.urbanPer
            element.urbanPer=urbanPer
            element.ruralPer=ruralPer
            break;
        }else{
